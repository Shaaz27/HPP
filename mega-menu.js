...
(function () {
  ...
  function buildMobileNav() {
    ...
    navData.forEach(function (l1) {
      html += '<div class="mobile-accordion__l1">';
      html += '<button class="mobile-accordion__trigger" aria-expanded="false">';
      html += esc(l1.label);
      html += chevronDown;
      html += '</button>';
      html += '<div class="mobile-accordion__panel" hidden>';

      if (l1.children) {
        l1.children.forEach(function (l2) {
          var hasChildren = l2.children && l2.children.length > 0;
          html += '<div class="mobile-accordion__l2">';
          html += '<button class="mobile-accordion__l2-trigger" aria-expanded="false">';
          html += esc(l2.label);
          if (hasChildren) html += chevronDown;
          html += '</button>';

          if (hasChildren) {
            html += '<div class="mobile-accordion__l3-panel" hidden><ul>';
            l2.children.forEach(function (l3) {
              html += '<li><a href="' + l3.href + '">' + esc(l3.label) + '</a></li>';
            });
            html += '</ul></div>';
          }
          html += '</div>';
      ...
      if (l1.viewAll) {
        html += '<a href="' + l1.viewAll.href + '" class="mobile-accordion__view-all">' + esc(l1.viewAll.label) + '</a>';
      }
      html += '</div></div>';
    });

    html += '<a href="#" class="mobile-accordion__link">Catalogue</a>';
  ...
  function bindMobileEvents() {
    ...
    mobileNavEl.querySelectorAll('.mobile-accordion__trigger').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        btn.classList.toggle('mobile-accordion__trigger--open', !isOpen);
      });
    });

    mobileNavEl.querySelectorAll('.mobile-accordion__l2-trigger').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var panel = btn.nextElementSibling;
        if (!panel) return;
        var isOpen = btn.getAttribute('aria-expanded') === 'true';
        ...
        panel.hidden = isOpen;
        panel.style.maxHeight = isOpen ? '0' : panel.scrollHeight + 'px';
        btn.classList.toggle('mobile-accordion__l2-trigger--open', !isOpen);
      });
    });
  }
